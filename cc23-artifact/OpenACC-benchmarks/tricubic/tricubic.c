...
#endif
int tricubic(int nx, int ny, int ns,
	real* u0, real* u1, real* a, real* b, real* c)
{
	...
	for (int k = 1; k < ns - 2; k++)
	{
		...
		for (int j = 1; j < ny - 2; j++)
		{
			...
			{
				real w1_a =  1.0/6.0                      * _A(a, i,j,k) * (_A(a, i,j,k)+1.0) * (_A(a, i,j,k)+2.0);
				real w2_a = -0.5     * (_A(a, i,j,k)-1.0)                * (_A(a, i,j,k)+1.0) * (_A(a, i,j,k)+2.0);
				real w3_a =  0.5     * (_A(a, i,j,k)-1.0) * _A(a, i,j,k)                      * (_A(a, i,j,k)+2.0);
				real w4_a = -1.0/6.0 * (_A(a, i,j,k)-1.0) * _A(a, i,j,k) * (_A(a, i,j,k)+1.0);

				real w1_b =  1.0/6.0                      * _A(b, i,j,k) * (_A(b, i,j,k)+1.0) * (_A(b, i,j,k)+2.0);
				real w2_b = -0.5     * (_A(b, i,j,k)-1.0)                * (_A(b, i,j,k)+1.0) * (_A(b, i,j,k)+2.0);
				real w3_b =  0.5     * (_A(b, i,j,k)-1.0) * _A(b, i,j,k)                      * (_A(b, i,j,k)+2.0);
				real w4_b = -1.0/6.0 * (_A(b, i,j,k)-1.0) * _A(b, i,j,k) * (_A(b, i,j,k)+1.0);

				real w1_c =  1.0/6.0                      * _A(c, i,j,k) * (_A(c, i,j,k)+1.0) * (_A(c, i,j,k)+2.0);
				real w2_c = -0.5     * (_A(c, i,j,k)-1.0)                * (_A(c, i,j,k)+1.0) * (_A(c, i,j,k)+2.0);
				real w3_c =  0.5     * (_A(c, i,j,k)-1.0) * _A(c, i,j,k)                      * (_A(c, i,j,k)+2.0);
				real w4_c = -1.0/6.0 * (_A(c, i,j,k)-1.0) * _A(c, i,j,k) * (_A(c, i,j,k)+1.0);
